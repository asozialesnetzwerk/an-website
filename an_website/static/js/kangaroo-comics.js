...
const days=["Sonntag","Montag","Dienstag","Mittwoch","Donnerstag","Freitag","Samstag",];const getDayName=(date)=>days[date.getDay()];const months=["Januar","Februar","März","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember",];const getMonthName=(date)=>months[date.getMonth()];const getDateString=(date)=>("Comic von "+
getDayName(date)+", dem "+
date.getDate()+". "+
getMonthName(date)+" "+
date.getFullYear());function removeAllPopups(){for(const node of d.getElementsByClassName("popup-container")){node.remove();}}
...
const firstDateWithOldLink=getDateBy(2020,12,3);const oldLinkRegex=/administratives\/kaenguru-comics\/kaenguru-(\d{2,3})(?:-2)?\/original\/?/;const firstDateWithNewLink=getDateBy(2021,1,19);const newLinkRegex=/administratives\/kaenguru-comics\/(\d{4})-(\d{2})\/(\d{2})\/original\/?/;const relativeLinkRegex=/\/static\/img\/(\d{4})-(\d{1,2})-(\d{1,2})\.jpg/;function getDateFromLink(link){for(const reg of[newLinkRegex,relativeLinkRegex]){const match=link.toLowerCase().match(reg);if(match&&match.length>3){return getDateBy(match[1],match[2],match[3]);}}
const arr=link.toLowerCase().match(oldLinkRegex);if(arr&&arr.length>1){const num=arr[1]-5;const date=new Date(firstDateWithOldLink.getTime());for(let i=0;i<num;i++){date.setTime(dateIncreaseByDays(date,isSunday(date)?2:1));}
return isSunday(date)?dateIncreaseByDays(date,1):date;}
link=link.toLowerCase().trim();switch(link){case"administratives/kaenguru-comics/pilot-kaenguru/original":return getDateBy(2020,11,29);case"administratives/kaenguru-comics/pow-kaenguru/original":return getDateBy(2020,11,30);case"static/img/kaenguru-announcement/original":return getDateBy(2020,11,30);case"administratives/kaenguru-comics/der-baum-kaenguru/original":return getDateBy(2020,12,1);case"administratives/kaenguru-comics/warnung-kaenguru/original":return getDateBy(2020,12,2);case"administratives/2020-12/kaenguru-comics-kaenguru-019/original":return getDateBy(2020,12,19);}
for(const arr of wrongLinks){if(link===arr[1])return arr[0];}}
const linkFormat="administratives/kaenguru-comics/%y-%m/%d/original";function generateComicLink(date){for(const arr of wrongLinks){if(datesEqual(date,arr[0]))return arr[1];}
const month=(date.getMonth()+1).toString();const day=date.getDate().toString();return linkFormat.replace("%y",date.getFullYear().toString()).replace("%m",month.length===2?month:"0"+month).replace("%d",day.length===2?day:"0"+day);}
function dateIncreaseByDays(date,days){date.setTime(date.getTime()+(days*1000*60*60*24),);date.setHours(6);return date;}
const comicCountToLoadOnCLick=7;const loadButton=elById("load-button");const list=elById("old-comics-list");let loaded=0;function loadMoreComics(){for(let i=0;i<comicCountToLoadOnCLick;i++){loaded++;const c=comics.length-loaded;if(c<0)break;let link=comics[c];const date=getDateFromLink(link);link=link.startsWith("/")?link:"https://img.zeit.de/"+link;const listItem=d.createElement("li");const header=d.createElement("a");header.classList.add("comic-header");header.innerText=getDateString(date)+":";header.href=link;header.style.fontSize="25px";listItem.appendChild(header);listItem.appendChild(d.createElement("br"));const image=d.createElement("img");image.classList.add("normal-img");image.src=link;image.alt=getDateString(date);image.onclick=()=>createImgPopup(image);image.onerror=()=>{if(isSunday(date)){list.removeChild(listItem);}else{listItem.append(" konnte nicht geladen werden.");}};listItem.appendChild(image);list.appendChild(listItem);}
if(loaded>=comics.length){loadButton.style.opacity="0";loadButton.style.visibility="invisible";}}
elById("load-button").onclick=loadMoreComics;function createImgPopup(image){removeAllPopups();const popupContainer=d.createElement("div");popupContainer.classList.add("popup-container");popupContainer.onmouseleave=()=>popupContainer.remove();popupContainer.onclick=()=>removeAllPopups();const clone=image.cloneNode(true);clone.classList.remove("normal-img");clone.classList.add("popup-img");const closeButton=d.createElement("img");closeButton.classList.add("close-button");closeButton.src="/static/img/close.svg?v=0";popupContainer.appendChild(clone);popupContainer.appendChild(closeButton);image.parentNode.appendChild(popupContainer);}
comics.push.apply(comics,links.split("\n"));addLinksToComics();const today=dateIncreaseByDays(getToday(),1);setCurrentComic(today);currentImg.onerror=(_e)=>{dateIncreaseByDays(today,-1);setCurrentComic(today);if(loaded<comicCountToLoadOnCLick){loaded++;}};}
