...
(()=>{const messageInput=elById("message-input");const messageSection=elById("message-section");const usingOpenMoji=elById("open-moji-attribution");const connectionIndicator=elById("connection-state");const currentUser=elById("current-user");let reconnectTimeout=100;let reconnectTries=0;let lastMessage="";const appendMessage=(msg)=>{let el=document.createElement("div");if(usingOpenMoji&&usingOpenMoji.getAttribute("type")!=="font"){for(let emoji of msg.author){el.append(emojiToIMG(emoji));}
...
+`//${w.location.host}/websocket/emoji-chat`);let pingInterval=setInterval(()=>ws.send(""),10_000);ws.onclose=(event)=>{messageInput.form.onsubmit=()=>{};if(event.wasClean){console.log(`[close] Connection closed cleanly, code=${event.code} reason=${event.reason}`);setConnectionState("disconnected");return;}
console.log(`Connection closed, reconnecting in ${reconnectTimeout}ms`);setConnectionState("connecting");clearInterval(pingInterval);if(reconnectTries>20){setConnectionState("disconnected");return}
setTimeout(()=>{reconnectTimeout=Math.max(500,Math.floor(Math.min(15000,1.5*reconnectTimeout-200)));reconnectTries++;openWS();},reconnectTimeout)};ws.onopen=(event)=>{console.log("Opened WebSocket",event);};ws.onmessage=handleWebsocketData;messageInput.form.onsubmit=(event)=>{if(messageInput.value!==""){lastMessage=messageInput.value;ws.send(JSON.stringify({"type":"message","message":messageInput.value,}));messageInput.value="";}
