...
(()=>{const textInput=document.getElementById("text");const configInput=document.getElementById("config-textarea");const outputText=document.getElementById("output");const errorText=document.getElementById("error_msg");function onerror(error){console.error(error);if(error.error){alert(error.error);errorText.innerText=`${error.error} In line ${error.line_num}: "${error.line}"`;}else{alert(error);errorText.innerText=error;}}
function ondata(data,onpopstate=false){if(!data){console.log("data is falsy!")
return;}
