...
"use strict";(()=>{let bigIntType="bigint";try{BigInt(69);}catch(_e){BigInt=Number;bigIntType="number";}
const output=elById("output");const fields=[elById("euro"),elById("mark"),elById("ost"),elById("schwarz"),];const factors=[BigInt(1),BigInt(2),BigInt(4),BigInt(20),];const numberRegex=/^(?:\d+|(?:\d+)?[,.]\d{1,2}|\d+[,.](?:\d{1,2})?)?$/;const isZero=(str)=>/^0*$/.test(str);function getDisplayValue(wert){if(typeof wert==="string"){wert=strToBigInt(wert);}
if(typeof wert!==bigIntType){alert(`Ungültiger Wert ${wert} mit type ${typeof wert}`);}
if(bigIntType==="number")wert=Math.floor(wert);let str=wert.toString();if(bigIntType==="number"&&str.includes("e")){const[num,pow]=str.split("e");if(pow.startsWith("-"))return"0";let[int,dec]=num.split(".");dec=dec||"";str=int+dec+"0".repeat(pow-dec.length);}
if(isZero(str))return"0";const dec=str.slice(-2);return((str.slice(0,-2)||"0")+
...
fields[3].value+" Ostmark auf dem Schwarzmarkt!";};function onSubmit(event){event.preventDefault();for(const feld of fields){feld.value=getDisplayValue(feld.value);}
setEuroParam(fields[0].value,true);updateOutput();}
