...
const output=elById("output");const fields=[elById("euro"),elById("mark"),elById("ost"),elById("schwarz")];const factors=[BigInt(1),BigInt(2),BigInt(4),BigInt(20)];const numberRegex=/^(?:\d+|(?:\d+)?[,.]\d{1,2}|\d+[,.](?:\d{1,2})?)?$/;function isZero(str){return/^0*$/.test(str);}
function getDisplayValue(wert){if(typeof wert==="string")
wert=strToBigInt(wert);if(typeof wert!==bigIntType)
alert(`Ungültiger Wert ${wert} mit type ${typeof wert}`);if(bigIntType==="number")wert=Math.floor(wert);let str=wert.toString();if(bigIntType==="number"&&str.includes("e")){let[num,pow]=str.split("e");if(pow.startsWith("-")){return"0";}else{let[int,dec]=num.split(".");dec=dec||"";str=int+dec+"0".repeat(pow-dec.length);}}
if(isZero(str))return"0";let dec=str.slice(-2);return((str.slice(0,-2)||"0")
+(isZero(dec)?"":","+(dec.length===1?"0":"")+dec));}
function strToBigInt(str){if(isZero(str))return BigInt(0);let[int,dec]=[str,"00"];if(str.includes(",")){[int,dec]=str.split(",");}else if(str.includes(".")){[int,dec]=str.split(".");}
