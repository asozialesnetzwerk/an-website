...
function dynLoadFixHref(anchor){if(anchor.target==="_blank")return;const href=(anchor.href.startsWith("/")?(w.location.origin+anchor.href):anchor.href).trim();const hrefWithoutQuery=href.split("?")[0];if(!href.startsWith(w.location.origin)||(hrefWithoutQuery.split("/").pop().includes(".")&&hrefWithoutQuery!==(w.location.origin+"/redirect"))||hrefWithoutQuery===(w.location.origin+"/chat")){return;}
if(href.startsWith("#")||href.startsWith(w.location.href.split("#")[0]+"#")){return;}
anchor.onclick=(e)=>{dynLoad(href);e.preventDefault();};}
