...
function dynLoadReplaceAnchors(){for(const anchor of document.getElementsByTagName("A")){const href=anchor.href.startsWith("/")?(window.location.origin+anchor.href):anchor.href;if(href.startsWith(window.location.origin)&&!(href.split("/").pop().includes("."))&&!(href.includes("#")&&(href.startsWith("#")||((!window.location.hash&&href.startsWith(window.location.href+"#"))||href.startsWith(window.location.origin
+window.location.pathname
+window.location.search
+"#"))))){const[requestUrl,params]=getJSONURLWithParams(href);anchor.href="javascript:void(0);";anchor.onclick=()=>{console.log("Processing anchor",href);history.replaceState({"data":window.urlData,"url":window.location.href,"scrollPos":[document.documentElement.scrollLeft||document.body.scrollLeft,document.documentElement.scrollTop||document.body.scrollTop],"stateType":"dynLoad"},document.title,window.location.href);if(href!==window.location.href){bodyDiv.innerHTML="Loading...";get(requestUrl,params,(data)=>dynLoadOnData(data,false),(error)=>{console.log(error);window.location.href=href;});}};}}}
function dynLoadOnPopState(event){if(event.state){console.log("Popstate",event.state);if(!(event.state["data"]&&dynLoadOnData(event.state,true))){const[requestUrl,params]=getJSONURLWithParams(event.state["url"]||window.location.href);bodyDiv.innerHTML="Loading... If this takes too long (over a few seconds), just refresh the page.";get(requestUrl,params,(data)=>dynLoadOnData(data,false),(error)=>{console.log(error);window.location.reload()});}
