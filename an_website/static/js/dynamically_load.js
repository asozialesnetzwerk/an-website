...
const title=data["title"];d.title=title;const shortTitle=data["short_title"]||title;let titleStyleText=`#title:before{content:"${shortTitle}"}`;if(shortTitle!==title){titleStyleText+=(`@media(min-width:500px){#title:before{content:"${title}"}}`);}
elById("title-style").innerText=titleStyleText;dynLoadReplaceAnchors();w.urlData=data;return true}
