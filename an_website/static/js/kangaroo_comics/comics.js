...
`;function B(){const i=f(),n=D(z);for(;n.getTime()<=i.getTime();)u.push(b(n)),g(n,1)}const E=["Sonntag","Montag","Dienstag","Mittwoch","Donnerstag","Freitag","Samstag"],F=i=>E[i.getDay()],N=["Januar","Februar","März","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember"],S=i=>N[i.getMonth()],d=i=>`Comic von ${F(i)}, dem ${i.getDate()}. ${S(i)} ${i.getFullYear()}`;function k(){for(const i of document.getElementsByClassName("popup-container"))i.remove()}const y=elById("current-comic-header"),v=elById("current-img");v.onmouseover=k;function L(i){let n=b(i);n=n.startsWith("/")?n:"https://img.zeit.de/"+n,v.src=n,y.innerText="Neuster "+d(i)+":",y.href=n}const x=e(2020,12,3),H=/administratives\/kaenguru-comics\/kaenguru-(\d{2,3})(?:-2)?\/original\/?/,z=e(2021,1,19),Y=/administratives\/kaenguru-comics\/(\d{4})-(\d{2})\/(\d{2})\/original\/?/,A=/\/static\/img\/(\d{4})-(\d{1,2})-(\d{1,2})\.jpg/;function W(i){for(const a of[Y,A]){const t=i.toLowerCase().match(a);if(t&&t.length>3)return e(parseInt(t[1]),parseInt(t[2]),parseInt(t[3]))}const n=i.toLowerCase().match(H);if(n&&n.length>1){const a=parseInt(n[1])-5,t=new Date(x.getTime());for(let r=0;r<a;r++)t.setTime(g(t,l(t)?2:1).getTime());return l(t)?g(t,1):t}switch(i=i.toLowerCase().trim(),i){case"administratives/kaenguru-comics/pilot-kaenguru/original":return e(2020,11,29);case"administratives/kaenguru-comics/pow-kaenguru/original":return e(2020,11,30);case"static/img/kaenguru-announcement/original":return e(2020,11,30);case"administratives/kaenguru-comics/der-baum-kaenguru/original":return e(2020,12,1);case"administratives/kaenguru-comics/warnung-kaenguru/original":return e(2020,12,2);case"administratives/2020-12/kaenguru-comics-kaenguru-019/original":return e(2020,12,19)}for(const a of o)if(i===a[1])return a[0];return null}const $="administratives/kaenguru-comics/%y-%m/%d/original";function b(i){for(const t of o)if(T(i,t[0]))return t[1];const n=(i.getMonth()+1).toString(),a=i.getDate().toString();return $.replace("%y",i.getFullYear().toString()).replace("%m",n.length===2?n:"0"+n).replace("%d",a.length===2?a:"0"+a)}function g(i,n){return i.setTime(i.getTime()+n*1e3*60*60*24),i.setHours(6),i}const C=7,I=elById("load-button"),w=elById("old-comics-list");let c=0;const J=()=>{for(let i=0;i<C;i++){c++;const n=u.length-c;if(n<0)break;let a=u[n];const t=W(a);if(t===null){console.error("No date found for "+a);continue}a=a.startsWith("/")?a:"https://img.zeit.de/"+a;const r=document.createElement("li"),m=document.createElement("a");m.classList.add("comic-header"),m.innerText=d(t)+":",m.href=a,m.style.fontSize="25px",r.appendChild(m),r.appendChild(document.createElement("br"));const s=document.createElement("img");s.classList.add("normal-img"),s.src=a,s.alt=d(t),s.onclick=()=>O(s),s.onerror=()=>{l(t)?w.removeChild(r):r.append(" konnte nicht geladen werden.")},r.appendChild(s),w.appendChild(r)}c>=u.length&&(I.style.opacity="0",I.style.visibility="invisible")};elById("load-button").onclick=J;const O=i=>{k();const n=document.createElement("div");n.classList.add("popup-container"),n.onmouseleave=()=>n.remove(),n.onclick=()=>k();const a=i.cloneNode(!0);a.classList.remove("normal-img"),a.classList.add("popup-img");const t=document.createElement("img");t.classList.add("close-button"),t.src="/static/img/close.svg?v=0",n.appendChild(a),n.appendChild(t),i.parentNode.appendChild(n)};u.concat(M.split(`
`)),B();const p=g(f(),1);L(p),v.onerror=()=>{g(p,-1),L(p),c<C&&c++}}(()=>{const e=elById("start-button-no_3rd_party");if(e!==null){const o=elById("comic-content-container");e.onclick=()=>{e.remove(),o.classList.remove("hidden"),startLoadingComics()},o.classList.add("hidden")}else startLoadingComics()})();// @license-end
