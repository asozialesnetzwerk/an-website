...
const snow=document.getElementById("snow");let snowflakesCount=200,bodyHeightPx,pageHeightVh;function setHeightVariables(){bodyHeightPx=document.documentElement.getBoundingClientRect().height,pageHeightVh=100*Math.max(bodyHeightPx/window.innerHeight,1)}function getSnowAttributes(){var n,t,e;snow&&(snowflakesCount=Number((e=(t=(n=snow.attributes)==null?void 0:n.count)==null?void 0:t.value)!=null?e:snowflakesCount))}function showSnow(n){n?snow.style.display="block":snow.style.display="none"}function spawnSnow(n=200){for(let t=1;t<=n;t++){const e=document.createElement("p");snow.appendChild(e)}}function addCss(n){const t=document.createElement("style");t.appendChild(document.createTextNode(n)),document.getElementsByTagName("head")[0].appendChild(t)}function randomInt(n=100){return Math.floor(Math.random()*n)+1}function randomIntRange(n,t){return n=Math.ceil(n),t=Math.floor(t),Math.floor(Math.random()*(t-n+1))+n}function getRandomArbitrary(n,t){return Math.random()*(t-n)+n}function spawnSnowCSS(n=200){let t="";for(let e=1;e<=n;e++){const o=Math.random()*100,r=Math.random()*10,s=o+r,d=o+r/2,l=getRandomArbitrary(.3,.8),i=l*100,a=Math.random(),c=randomIntRange(10,30),m=randomInt(30)*-1,u=Math.random();t+=`
      ...
      @keyframes fall-${e} {
        ${l*100}% {
          transform: translate(${s}vw, ${i}vh) scale(${a});
        }
        to {
