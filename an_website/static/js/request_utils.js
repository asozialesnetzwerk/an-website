...
window.PopStateHandlers={"replaceURL":(state)=>{window.lastLocation===state["origin"]||window.location.reload();},"URLParamChange":(state)=>{window.location.reload();}};function setURLParam(param,value,state,stateType="URLParamChange",push=true){console.log("setURLParam",param,value,state,onpopstate);const urlParams=new URLSearchParams(window.location.search);urlParams.set(param,value);const newUrl=`${window.location.origin}${window.location.pathname}?${urlParams.toString()}`;console.log("newUrl",newUrl);state["stateType"]=stateType;if(push){history.pushState(state,newUrl,newUrl);}else{history.replaceState(state,newUrl,newUrl)}
return newUrl;}
