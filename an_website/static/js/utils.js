...
"use strict";"use strict";const w=window;const d=document;const elById=(id)=>d.getElementById(id);const log=console.log;const error=console.error;w.lastLocation=String(w.location);function post(url,params={},ondata=log,onerror=error){fetch(url,{method:"POST",body:JSON.stringify(params),headers:{"Accept":"application/json","Content-Type":"application/json"}}).then(response=>response.json()).catch(onerror).then(ondata).catch(onerror);}
function get(url,params={},ondata=log,onerror=error){if(params)url+="?"+(new URLSearchParams(params)).toString()
fetch(url,{method:"GET",headers:{"Accept":"application/json"}}).then(response=>response.json()).catch(onerror).then(ondata).catch(onerror);}
