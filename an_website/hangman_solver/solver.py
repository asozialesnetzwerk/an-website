...
async def generate_pattern_str(input_str: str, invalid: str, crossword_mode: bool) -> str:
    ...
    return WILDCARDS_REGEX.sub(lambda m: (wild_card_replacement + "{" + str(length_of_match(m)) +
                                          "}"), input_str)


async def get_words_and_letters(file_name: str, input_str: str, invalid: str,
                                crossword_mode: bool) -> tuple[list[str], dict[str, int]]:
    ...
    sorted_letters: List[Tuple[str, int]] = sorted(letters_items, key=lambda item: item[1],
                                                   reverse=True)
